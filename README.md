...
Généralités concernant bash (cf recapitulatif papier jour1), prise en main de bash.
...
- Les paires de bases de fin de reads présentent des scores de qualité assez bas.
- Les paires de bases de début de reads possèdent un contenu en ACGT anormal.
...
- MINLEN:100 qui permet de ne garder que les reads de taille supérieure à 100pb après correction (pour éviter de se retrouver avec des petits reads problématiques pour l'assemblage)
...
Premières recherches sur TransDecoder, les fichiers cds d'intérêt et la manière de construire une banque blast, en prévision de la session de décembre !
...
Suite à cela, nous allons construire une blast data bank à partir de nos séquences et blaster les cds d'ISG humains contre cette banque. Nous choisissons d'utiliser les cds humains car nous disposons de données très complètes et bien annotées chez l'homme. Il aurait également été possible de blaster les cds d'une espèce de chauve-souris proche de *Myotis velifer* (par exemple *Myotis lucifugus*). Cette étape nous permet de récupérer les homologues d'ISG chez *Myotis velifer*.

Pour le blast, nous utilisons le script use_makeblastdb_blastn qui permet de construire la database via makeblastdb et de blaster nos séquences d'intêtet contre cette database via blastn.
...
On va commencer par écrire un script pour ne garder que les séquences dont la taille correspond au moins à la moitié de la taille de la séquence de référence. Le but est d'éliminer les artefacts d'assemblage de petite taille. Pour cela, nous utilisons le script parse_blastn_output.sh. On utilise la fonction blastdbcmd afin d'extraire les homologues d'intéret de notre database.
...
Maintenant que nous avons récupéré les différents homologues, nous allons réaliser l'alignement des séquences en utilisant PRANK.
...
Les sorties de Prank sont converties au format phylip en utilisant trimAl (use_trimal.sh) grace à l'option -phylip. L'utilisation de ce format est necessaire pour PhyML.
...
- -m : Choix du modèle de substitution des acides nucléiques, ici, le modèle HKY85. Le modèle choisit fait la distinction entre les transitions et les transversions et donne une probabilité différente aux différentes bases (ACGT).
...
Nous ne présenterons pas ici tous les résultats de manière détaillée pour des raisons de protection des données. Ceux-ci ont déjà été présentés aux personnes concernées lors des différentes présentations réalisées en cours et en fin de TP.

Les ISG suivants ont été testés avec la méthode décrite precedemment :
- BST2
- PKR
- SAMD9-SAMD9L
...
Artefacts observés :
- Dans certains cas, l'assemblage par trinity va être responsable de la séparation de plusieurs transcrits issus d'un même gène qui sont alors identifiés comme des homologues différents. Dans ce cas, nous obtenons de très nombreux homologues pour un même gène ce qui est un artefact d'assemblage. C'est par exemple le cas avec BST2 pour lequel nous obtenons 11 homologues différents pour *Myotis velifer*. Une méthode pour contrer ce genre d'artefact serait l'utilisation d'un génome de référence pour repérer ces homologues artefactuels. (nous ne disposons pas de génome de référence dans le cas de *Myotis velifer* mais celui d'une espèce proche pourrait être utilisé avec le risque de rater les homologues propres à *Myotis velifer*)
- Nous observons également des phénomènes d'attraction des longues branches avec des problèmes d'enracinement des arbres (notamment pour SAMD9-SAMD9L). Cela peut être corrigé en changeant l'enracinement des arbres.



Plusieurs phénomènes évolutifs interessants ont été observés et peuvent être mentionnés ici :
- Des phénomènes de duplications anciennes communs à l'ensemble des espèces de chauve-souris étudiées notamment dans le cas de SAMD9-SAMD9L.
- Des phénomènes de duplications récentes avec notamment dans le cas de PKR une duplication récente propre aux *Myotis*.

Ces phénomènes évolutifs peuvent être le marqueur d'une course à l'armement entre le système immunitaire des chauves-souris et les virus. Avec les données analysées, nous pouvons comprendre que ces phénomènes ont une origine commune à l'ensemble des chauves-souris, tout en continuant à prendre place à l'échelle des différentes espèces, notamment *Myotis velifer*.



# Le mot de la fin

En cas de problème lors de l'utilisation des scripts ou des informations mises à disposition sur ce github, n'hésitez pas à prendre contact avec l'auteur.
